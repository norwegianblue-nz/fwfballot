  ...
  public static function processIdentityElement(array &$element, FormStateInterface $form_state, &$complete_form) {
    ...
    if (!$change_it) {
      ...
      if (count($people) > 0) {
        ...
        $element['for']['fortext']['#markup'] = ((string) t('Hunting party includes:')) . ' ' . implode(', ', $people_labels);
    ...
    $element['entities'] = [
      ...
      '#open' => TRUE,
      '#tree' => TRUE,
      '#title' => $arity_is_multiple ? t('Add another hunter') : t('Select person'),
      '#attributes' => [
        'class' => ['entities'],
      ],
    ];
...
// nb //    if ($arity_is_multiple && count($people) > 0) {
// nb //      $element['entities']['controls']['actions']['done'] = [
// nb //        '#type' => 'submit',
// nb //        '#value' => t('Done'),
// nb //        '#ajax' => [
// nb //          'callback' => [static::class, 'ajaxElementRoot'],
// nb //          'wrapper' => $ajax_wrapper_id_root,
// nb //        ],
// nb //        '#limit_validation_errors' => [],
// nb //        '#validate' => [
// nb //          [static::class, 'decoyValidator'],
// nb //        ],
// nb //        '#submit' => [
// nb //          [static::class, 'submitClose'],
// nb //        ],
// nb //      ];
// nb //    }
...
// nb //          '#title' => t('Existing @entity_type', ['@entity_type' => $entity_type->getLabel()]),
          '#title' => t('', ['@entity_type' => $entity_type->getLabel()]),
  ...
  public static function validateRegistrantCount(&$element, FormStateInterface $form_state, &$complete_form) {
    ...
    if (isset($element['#registrants_minimum'])) {
      $minimum = $element['#registrants_minimum'];
      if ($count < $minimum) {
        $form_state->setError($element, t('There are not enough members in this hunting party. There must be at least @minimum members.', [
          '@minimum' => $minimum,
        ]));
      }
    }
    ...
    if (isset($element['#registrants_maximum'])) {
      $maximum = $element['#registrants_maximum'];
      if ($count > $maximum) {
        $form_state->setError($element, t('There are too many members in this hunting party. There must be at most @maximum members.', [
          '@maximum' => $maximum,
        ]));
      }
    }
    // norwegian.blue - check that hunting party not 5 members
    $blacklistnum = 5;
    if ($count == $blacklistnum) {
      $form_state->setError($element, t('We are sorry, but hunting parties cannot consist of @blacklistnum  members.', [
